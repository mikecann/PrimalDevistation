...
sampler AgeSampler = sampler_state
{
    Texture = (AgeTexture);
    magfilter=point;
    minfilter=point;
...
{   
	...
	else
	{
		...
		Velocity = applyTerrainCollisionVel(Position,Velocity,texCoord);
	}
...
{  
	float4 Position = tex2D(PositionSampler, texCoord);	
	float4 Velocity = tex2D(VelocitySampler, texCoord);
	float4 OutCol = Position;
	if (Velocity.x !=0 && Velocity.y!=0){ OutCol=applyTerrainCollisionPos(Position,Velocity,texCoord); }
	return OutCol;
}

float4 PSUpdateAge(float2 texCoord : TEXCOORD0) : COLOR0
{  	
	float4 age = tex2D(AgeSampler, texCoord);	
	age.x += FrameDelta;
	return age; 
}
...
technique UpdateAge
{
    pass Pass1
    {
		...
		ZWriteEnable = false;
		VertexShader = compile vs_3_0 VertexShader();
        PixelShader = compile ps_3_0 PSUpdateAge();
    }
}
